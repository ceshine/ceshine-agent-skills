...
1. **No staged changes**: Run `scripts/git-cached-diff.sh`. If output is non-empty, **STOP** and tell the user:
...
2. **Changes exist**: Run `scripts/git-unstaged-diff.sh` and `git ls-files --others --exclude-standard`. If both are empty, inform the user there is nothing to commit.
...
- **Modified/deleted files**: `scripts/git-unstaged-diff.sh` (full patch output)
- **Untracked files**: `git ls-files --others --exclude-standard`
- **Rename detection**: `git diff -M --diff-filter=R --name-status` (no script wrapper — this is a specialized query)
...
Run `scripts/git-cached-diff.sh`. Must be empty. If not, **STOP** — something went wrong.
...
Run `scripts/git-cached-diff.sh` and confirm only the intended files/hunks are staged.
...
- Load the two most recent commit messages (`scripts/git-commit-messages.sh HEAD~2`) for style reference
...
- **Hunk staging produces unexpected results**: Show `scripts/git-cached-diff.sh` output. Ask user for guidance.
